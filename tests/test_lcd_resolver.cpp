...
{
    const plot::lcd_resolver_probes_t probes;
    TEST_ASSERT(
        plot::resolve_lcd_subpixel_order_from_probes(
...
bool test_auto_prefers_qt_probe_and_skips_windows()
{
    ...
    plot::lcd_resolver_probes_t probes;
    probes.qt_probe = [] {
...
bool test_explicit_orders_skip_probes()
{
    ...
    for (const explicit_request_case_t& item : explicit_orders) {
        TEST_ASSERT(
            plot::resolve_lcd_subpixel_order_from_probes(item.request, probes) ==
                item.expected,
...
bool test_explicit_orders_for_null_window_are_deterministic()
{
    ...
    for (const explicit_request_case_t& item : explicit_orders) {
        TEST_ASSERT(
            plot::resolve_lcd_subpixel_order_for_window(item.request, nullptr) ==
                item.expected,
...
{
    TEST_ASSERT(
        plot::lcd_from_qt_subpixel_hint(
    ...
    TEST_ASSERT(
        plot::lcd_from_qt_subpixel_hint(255) ==
            resolved_t::NONE,
...
{
    TEST_ASSERT(
        plot::lcd_from_windows_font_smoothing_settings(
            false,
