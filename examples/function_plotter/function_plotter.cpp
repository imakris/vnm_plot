...
bool use_scientific_for_range(double range, int digits)
{
    ...
    const std::string fixed_a = vnm::plot::algo::format_axis_fixed_or_int(a, digits);
    const std::string fixed_b = vnm::plot::algo::format_axis_fixed_or_int(b, digits);
    const std::string fixed_c = vnm::plot::algo::format_axis_fixed_or_int(c, digits);
...
void Function_plotter::configure_plot_widget()
{
    ...
    config.format_timestamp = [](double x, double range) -> std::string {
        ...
        return vnm::plot::algo::format_axis_fixed_or_int(x, digits);
    };
...
QVariantList Function_plotter::get_samples_at_x(double x, double plot_width, double plot_height) const
{
    QVariantList result;

    if (!m_plot_widget || plot_width <= 0 || plot_height <= 0) {
        return result;
    }

    // Get the current visible ranges from the plot widget
    const double t_min = m_plot_widget->t_min();
    const double t_max = m_plot_widget->t_max();
    const float v_min = m_plot_widget->v_min();
    const float v_max = m_plot_widget->v_max();

    const double t_span = t_max - t_min;
    const float v_span = v_max - v_min;

    if (t_span <= 0 || v_span <= 0) {
        return result;
    }
    ...
    for (const auto& entry : m_entries) {
        ...
        auto* data_source = entry->data_source();
        if (!data_source) {
            continue;
        }

        const auto& samples = data_source->data();
        const std::size_t count = samples.size();
        if (count == 0) {
            continue;
        }

        const bool ascending = samples.front().x <= samples.back().x;

        // Binary search for the nearest sample (account for order)
        std::size_t lo = 0;
        std::size_t hi = count - 1;

        while (lo < hi) {
            std::size_t mid = (lo + hi) / 2;
            if (ascending ? (samples[mid].x < x) : (samples[mid].x > x)) {
                lo = mid + 1;
            }
            else {
                hi = mid;
        ...
        std::size_t i1 = 0;
        if (count > 1) {
            const double first_x = samples.front().x;
            const double last_x = samples.back().x;
            if (ascending) {
                if (x <= first_x) {
                    i0 = 0;
                    i1 = 0;
        ...
        const double x0 = samples[i0].x;
        const double x1 = samples[i1].x;
        const double y0 = static_cast<double>(samples[i0].y);
        const double y1 = static_cast<double>(samples[i1].y);
        ...
        // X: map from [t_min, t_max] to [0, plot_width]
        double px = (x - t_min) / t_span * plot_width;

        // Y: map from [v_min, v_max] to [plot_height, 0] (inverted because screen Y is down)
        double py = (1.0 - (y - v_min) / v_span) * plot_height;

        // Clamp to valid range
        px = std::max(0.0, std::min(px, plot_width));
        py = std::max(0.0, std::min(py, plot_height));

        QVariantMap entry_data;
        entry_data["x"] = x;
        entry_data["y"] = y;
        entry_data["px"] = px;
        entry_data["py"] = py;
        entry_data["color"] = entry->color();
