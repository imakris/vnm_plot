...
The `049587c` native Unix rerun exposed hosted software-renderer memory pressure during backend creation, not a product requirement for a particular graphics API. At `86ad4c0`, Windows and macOS passed text OFF/ON and Linux text OFF passed, but Linux text ON still exhausted LLVM memory with one llvmpipe worker; the attempted FreeBSD NFS workspace also triggered correct Git ownership rejection before compilation. A subsequent softpipe experiment removed the OOM but was rejected because it could not consume the baked GLSL 410 shader path and produced a clear-only frame. The next attempt keeps Qt's native non-Null selection, the default Mesa renderer with one worker, a single-sample fallback/context surface, and the measured 4x offscreen render buffer. `GALLIUM_DRIVER`, `LP_NUM_THREADS`, and both sample counts remain retained fingerprint fields even when the driver override is unset. FreeBSD evidence packaging occurs inside the rsync-isolated guest; the guest writes a copied-back status marker and exits cleanly for copyback, then the host uploads evidence before propagating the recorded failure. The gate also creates a synthetic-source terminal FAIL manifest and diagnostic log when source identity or preflight itself fails, and requires preflight/invocation/raw renderer settings to agree.
