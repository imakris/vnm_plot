...
void Plot_widget::set_vbar_width_from_renderer(double px)
{
    ...
    if (std::abs(target - current) <= constants::k_vbar_width_change_threshold_d &&
...
{
    const float min_span = algo::min_v_span_for(target_vmin, target_vmax);
...
void Plot_widget::auto_adjust_view(bool adjust_t, double extra_v_scale, bool anchor_zero)
{
    ...
    const double min_span = static_cast<double>(
        algo::min_v_span_for(static_cast<float>(agg.vmin), static_cast<float>(agg.vmax)));
    if (!(span > min_span)) {
