...
text_lcd_subpixel_order_t sanitize_probe_result(text_lcd_subpixel_order_t order)
{
    return text_lcd_subpixel_order_is_display_specific(order)
        ? order
        : text_lcd_subpixel_order_t::NONE;
}

text_lcd_subpixel_order_t text_lcd_subpixel_order_from_qt(QQuickWindow* window)
{
    if (window == nullptr) {
...
text_lcd_subpixel_order_t text_lcd_subpixel_order_from_windows()
{
#if defined(_WIN32)
...
text_lcd_subpixel_order_t resolve_text_lcd_subpixel_order_from_probes(
    ...
    const text_lcd_resolver_probes_t& probes)
{
    ...
    const text_lcd_subpixel_order_t qt_order = probes.qt_probe
        ? sanitize_probe_result(probes.qt_probe())
        : text_lcd_subpixel_order_t::NONE;
    if (text_lcd_subpixel_order_is_display_specific(qt_order)) {
        return qt_order;
    }

    const text_lcd_subpixel_order_t windows_order = probes.windows_probe
        ? sanitize_probe_result(probes.windows_probe())
        : text_lcd_subpixel_order_t::NONE;
    return text_lcd_auto_order_from_detections(qt_order, windows_order);
}

text_lcd_subpixel_order_t resolve_text_lcd_subpixel_order_for_window(
    text_lcd_subpixel_order_t requested,
    QQuickWindow* window)
{
...
text_lcd_subpixel_order_t text_lcd_from_qt_subpixel_hint(int qt_subpixel_hint)
{
    switch (qt_subpixel_hint) {
        case static_cast<int>(QPlatformScreen::Subpixel_RGB):
            return text_lcd_subpixel_order_t::RGB;
        case static_cast<int>(QPlatformScreen::Subpixel_BGR):
            return text_lcd_subpixel_order_t::BGR;
        case static_cast<int>(QPlatformScreen::Subpixel_VRGB):
            return text_lcd_subpixel_order_t::VRGB;
        case static_cast<int>(QPlatformScreen::Subpixel_VBGR):
            return text_lcd_subpixel_order_t::VBGR;
...
text_lcd_subpixel_order_t text_lcd_from_windows_font_smoothing_settings(
    bool enabled,
