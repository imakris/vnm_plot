...
9. At `049587c`, Windows and macOS CI passed while Linux run `29107308679` and FreeBSD run `29107308656` reached native QRhi under Xvfb but aborted during `cold.backend_create` with LLVM out-of-memory/`std::bad_alloc`. Their failed smoke traces and gate manifests were retained. The failure reporter then exposed a separate missing `sys` import while reporting the original process failure. The import is fixed; the next CI attempt keeps native QRhi and bounds Mesa's hosted software-renderer worker count with `LP_NUM_THREADS=1` to fit the runners' memory limits. This infrastructure hypothesis remains open until that attempt passes.
