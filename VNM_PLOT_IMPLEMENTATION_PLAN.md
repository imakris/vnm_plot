...
|---|---|---|---|
| D1 | Negative values | Algebraic cumulative stacking; a negative component descends from the previous cumulative value, preserving `h=f+g`. | **Approved 2026-07-10.** P-R1's precise emitted-record/cursor evaluator remains pending; after ratification retain absorption, signed-zero, cancellation, negative, and ordinary-rounding verification. |
...
| D9 | Pixel-parity indicators and frame result | Batch 3B defines the canonical evaluator and value result. Batch 3C's single production frame entry creates a snapshot-free immutable ordinary frame plan and, after renderer/RHI disposition is known, one immutable ordinary frame result. The result copies frame/config/view identities from the plan and contains per-series status, sequence, selected LOD, actual disposition, rendered range, and value-only indicators tagged with the current cursor request ID. The GUI rejects a result whose request ID is not current. No independent indicator snapshot path exists; Batch 3D may optimize copying/publication but may not add a second producer. This ordinary D9 result is complete without P-R1/P-D15 or any stack section. | **Amended and approved 2026-07-11.** Retain plan/result construction, one-frame latency, current-request rejection, disposition, and ordinary-with-stack-fault independence tests. |
...
- **P-C1:** monotonic cursor-state publication, current/presented identities,
  immediate activation/movement/deactivation clearing, absent/failure currency,
  retained stale identity, and the complete GUI transition matrix;
- **P-R1:** self-describing ordered stack results, factory-enforced
  disposition/payload invariants, four terminal tagged source observations,
...
#### P-C1 — Cursor-state publication and GUI transition

P-C1 is a pending proposal, not an approved amendment to D9. The GUI owns one
monotonic `cursor_state_id`. Activation, every movement/request replacement,
and deactivation atomically increment it and immediately clear/hide prior
indicator tooltips, markers, and totals before requesting new work. A matching
later result may repopulate them; a mismatch performs no mutation and cannot
restore cleared state.

Every executed frame plan and enclosing D9 result copy the current-attempt
`cursor_state_id`. Presented READY content carries `content_cursor_state_id`
from the exact executed plan. Retained STALE content keeps the old
`content_cursor_state_id` unchanged even when its optional indicator batch is
absent; a newer attempt or deactivation never rewrites retained identity. A
present indicator batch's request ID must equal its presented
`content_cursor_state_id` and is unconstructible otherwise.

GUI result consumption applies this order after the event-side immediate clear:

1. Reject stale enclosing config identity without mutation.
2. Read current cursor activation independently of result payload. Inactive
   state stays clear/hidden and ignores every delayed or retained result.
3. If active, select the applicable currency before any result-driven mutation:
   presented READY/STALE uses `content_cursor_state_id`; EMPTY/FAILED and an
   enclosing result-storage failure use current-attempt `cursor_state_id`.
   Mismatch performs no apply or clear.
4. On a match, READY-status presented data replaces the complete indicator
   state, EMPTY-status or absent presented payload clears/hides it, and matching
   EMPTY/FAILED/result-storage failure clears its complete governed scope.

Tests begin with displayed content and cover activation, movement/replacement,
and deactivation; rapid A/B/C requests with every result order; old absent
payload, group failure, and result-storage failure; movement followed by BUSY
stale; matching/mismatching retained stale; and proof that delayed content never
reappears after immediate clear/deactivation.
...
Before any table or stack source allocation, one allocation-free metadata
prepass completes the deterministic logical map of every group/view/member
position: all MAIN groups by `lowest_enabled_series_id`, then PREVIEW, with
members by `series_id`. It performs registry validation in P-R1 precedence and
P-D15's fixed-slice admission. A registry-invalid unit receives its canonical
registry `FAILED(reason)`; a checked reservation that is invalid/overflows or
does not fit receives `FAILED(FRAME_BUDGET)`. The logical map is a deterministic
replay of immutable registry metadata and admission arithmetic, not an allocated
map container. This prepass performs no source acquisition, unit-owned staging,
geometry, QRhi, retained-entry, or public-table allocation.

Only after that complete map exists, and still before acquisition, calculate
and attempt one frame-wide `stack_result_counted_backing_t` allocation
containing every mapped group/view/member plus its exact counted header/padding.
The shared publication allocation is accounted once for the frame and never
attributed to an admitted or rejected unit. A configured-cap rejection sets the
enclosing section to `FAILED(RESULT_STORAGE_BUDGET)`; a cap-admitted allocator
failure sets it to `FAILED(RESULT_STORAGE_ALLOCATION_FAILED)`. Either stack-
section failure has no stack table, performs no stack acquisition or geometry
work, and clears all stack retained-content eligibility before publication.
Ordinary work continues unchanged.

After successful allocation, replay the map to populate every pre-acquisition
failure and member origin, then process only admitted units. A rejected unit has
no source acquisition or unit-owned staging/geometry/QRhi/retained allocation;
the independently accounted frame table is not a unit allocation. Phase
instrumentation must prove `metadata/admission < table allocation < acquisition`
and zero unit-owned allocation for every rejected unit. The per-entry P-R1
invariants below apply only when the stack section is `COMPLETE`.
...
`series_id`, including records belonging to a group/view whose disposition is
`FAILED(reason)` and records whose origin is `NOT_EVALUATED`. Per-series records
never have a `FAILED(reason)` disposition. These IDs are self-description labels
only and never cache, retention, or reuse authority.
...
origin-tagged terminal-attempted or selected LOD/logical window; rendered range;
`optional<stack_indicator_batch_t>`; and optional `content_frame_id`. It
also carries `content_cursor_state_id` exactly for READY/STALE_BUSY presented
content. It contains no geometry, spans, samples, resources, source identities,
owner handles, cache keys, or other private backing.

The public `Stack_failure_reason` is closed to exactly:

```text
enum class Stack_failure_reason {
    STACK_REQUIRES_TWO_MEMBERS,
    STACK_SOURCE_MISSING,
    CUSTOM_LAYER_IN_STACK,
    STACK_REQUIRES_TIMESTAMP,
    STACK_REQUIRES_SCALAR_VALUE,
    STACK_GROUP_HAS_NO_VISIBLE_STYLE,
    STACK_BUDGET_TOO_SMALL,
    FRAME_BUDGET,
    SOURCE_FAILED,
    SOURCE_BUSY,
    UNORDERED_STACK_INPUT,
    LOD_BUDGET_EXCEEDED,
    NONFINITE_REJECTED,
    FRAGMENTATION_BUDGET,
    STACK_NUMERIC_OVERFLOW,
    STACK_GLOBAL_RANGE_UNBOUNDED,
    STACK_OUTPUT_BUDGET_EXCEEDED,
    RESIDENT_BUDGET,
    RESOURCE_ALLOCATION_FAILED,
};
```

`failed(reason,observations)` accepts only `Stack_failure_reason`. The section
dispositions `FAILED(RESULT_STORAGE_BUDGET)` and
`FAILED(RESULT_STORAGE_ALLOCATION_FAILED)` are not enum members, never appear as
entry failure reasons, and remain exclusive to `stack_result_section_t`.

For each emitted composition position/member, private
`checked_cumulative_step(series_id,base_y,component_y)` is the sole cumulative
arithmetic primitive. It requires finite input values, performs the one ordinary
binary64 addition `base_y+component_y` used by the stack plan, requires the
result finite, and returns one private immutable
`{series_id,component_y,base_y,cumulative_y}` record. It preserves both input
values bitwise; in particular, absorption such as `1e16+1` may round the
cumulative back to `1e16` without changing the recorded component from `1`.
Each emitted position's ordered records enter the private immutable presented
sequence. Geometry/GPU conversion and VISIBLE range consume those exact emitted
records; neither reconstructs component, base, or cumulative or performs a
second cumulative addition.
...
Only these factory-enforced variants exist:

- The cursor evaluator never inserts an emitted position, changes geometry,
  consumes `M`/`V_limit`/`H_limit`, changes admission/acquisition, or queries raw
  source/pre-grid values. It samples only the already-frozen private presented
  sequence under the selected candidate's exact representation rules. Candidate
  A uses its bounded normalized/event-union interpolation, duplicate/nonfinite,
  gap, and hold rules. Candidate B uses the finalized bounded grid approximation,
  including exact interpolation on that grid and its displaced STEP transitions,
  never the pre-grid curve. At an exact emitted position it may reuse the stored
  record. Between positions it interpolates the stored `component_y`, `base_y`,
  and `cumulative_y` independently under those same frozen rules, with no
  cumulative addition or field reconstruction. The private immutable cursor-
  sampled records must have canonical `+0.0` first base, each later base bitwise
  equal to the preceding sampled cumulative, and final total equal to the last
  cumulative. Outside the frozen common domain or in a frozen gap, the complete
  group is all-EMPTY.
- private/frame-producer-only
  `stack_indicator_batch_t::ready(cursor_request_id,cursor_sampled_records)`
  accepts exactly `K` ascending private cursor-sampled records from the frozen
  executed or retained representation. It verifies the first `base_y` is
  canonical `+0.0` and every later `base_y` is bitwise the preceding record's
  `cumulative_y`, then copies record `component_y`/`base_y`/`cumulative_y` into
  public `component_y`/`stack_base_y`/`cumulative_y` and makes `stack_total_y`
  bitwise the final record's `cumulative_y`. Public callers
  cannot construct, supply, or manufacture the private records or independently
  supply component/base/cumulative/total fields;
...
The emitted-position cumulative primitive's one addition uses the
implementation's ordinary `double` operation; it is not repeated, algebraically
reconstructed, contracted with adjacent work, or normalized for signed zero.
Cursor sampling performs no cumulative addition. Mixed READY/EMPTY items,
publicly manufactured/inconsistent records, a nonfinite input/result,
duplicate/missing/extra member, and a zero-length present batch are
unconstructible. This uniform status follows the common published stack domain,
so a gap or out-of-domain cursor empties the complete group rather than a
filtered member subset.
...
ID. Matching READY/EMPTY/no-batch behavior governs one complete stack group,
matching group EMPTY/FAILED clears that group, and matching section failure
clears all stack groups. P-C1's config-first, immediate event-side clear,
inactive ignore, applicable-ID comparison, mismatch-no-mutation, and rapid-
request guarantees apply without modification. This P-R1 integration text does
not ratify P-C1.
...
- `ready(presented_payload)` and `stale_busy(retained)` require
  `content_frame_id`, `content_cursor_state_id`, rendered range, and the plan's
  exact `optional<stack_indicator_batch_t>` under the presence/item invariants
  above;
- `empty(observations)` and `failed(reason,observations)` contain no
  `content_frame_id`, `content_cursor_state_id`, rendered range, or indicator
  batch;
- `not_evaluated(series_id)` contains only its structural label;
- `observed_ready(series_id,terminal_attempted_lod,sequence,
  optional_logical_window)` is constructible once the controlling READY
  acquisition attempt/consume scope has terminated and its hold has been
  released/destroyed.
  ...
  succeeded. It carries the terminal attempted LOD and observed sequence; the
  optional logical window is present only if successfully derived. Prospective
  visit exhaustion, order/window failure, LOD-budget failure that prevents
  selection, or another later failure still retains this terminal observation
  after release, with no fabricated value or presented-content fact;
- `observed_empty(series_id,terminal_attempted_lod,sequence)` carries the exact
  controlling EMPTY outcome and observed sequence, including zero, with no
  window;
- `observed_busy(series_id,terminal_attempted_lod)` and
  `observed_failed(series_id,terminal_attempted_lod)` carry the controlling
  terminal BUSY/FAILED outcome, sequence zero, and no window;
- `presented_content(...)` is constructible only inside `READY`/`STALE_BUSY` with
  the enclosing entry's exact `content_frame_id` and
  `content_cursor_state_id`; it carries the selected LOD and logical window from
  the complete presented plan, never an attempted LOD.

The one public controlling/terminal source outcome for a member is
PRESENTED_CONTENT's selected READY only when the final group/view disposition is
READY, or retained PRESENTED_CONTENT when it is STALE_BUSY. Every final EMPTY or
`FAILED(reason)` instead projects each acquired source as its single terminal
CURRENT_OBSERVATION—even when READY selection and a logical window succeeded
before a later intersection/nonfinite/fragmentation/numeric/output/resident/RHI
failure. The optional logical window remains allowed when actually derived, but
there is no content frame/cursor ID, value, rendered range, or indicator batch.
Earlier LOD attempts and every visit counter remain internal diagnostic trace/profiler
records tagged by group, view, series, and attempt; they are not public result
fields. No provisional PRESENTED_CONTENT is constructed before final
disposition selection.

The CURRENT_OBSERVATION source-status domain is the existing closed
`snapshot_result_t::Snapshot_status`: `READY`, `EMPTY`, `BUSY`, and `FAILED`.
`UNSUPPORTED` is not a completed stack acquisition and cannot construct
CURRENT_OBSERVATION. The four factories above distinguish attempted source LOD
from PRESENTED_CONTENT's selected LOD/window and make every other status/
sequence/window combination unconstructible.
...
- `CURRENT_OBSERVATION` is one of the four exact `observed_*` terminal-outcome
  variants above, is associated with the enclosing result's
  `publication_frame_id`, and carries no value, indicator batch, rendered range,
  `content_frame_id`, or `content_cursor_state_id`;
- `PRESENTED_CONTENT` carries only complete selected-plan fields associated with
  the entry's exact `content_frame_id`/`content_cursor_state_id`, including
  selected rather than attempted LOD/window.
...
range, or indicator batch and may contain only each actually acquired member's
single controlling terminal CURRENT_OBSERVATION. Every pre-acquisition failure
has all entries NOT_EVALUATED. A partial acquisition failure has one terminal
CURRENT_OBSERVATION for each member completed through the first FAILED and
NOT_EVALUATED afterward. A source that reached selected READY still projects to
terminal `observed_ready` if the final group is EMPTY or fails during later
normalization, intersection, fragmentation, numeric conversion, output budget,
resident transaction, CPU allocation, or RHI allocation; its derived optional
window is preserved without any presented-content field.
...
fields, rendered range, the complete optional indicator batch, and
`content_frame_id`/`content_cursor_state_id`—into the current independent public
stack table. No geometry or private key is copied into that table, and no
current partial acquisition enters presentation; current attempts exist only in
diagnostic trace/counters. The normal GUI
cursor-state check therefore rejects old retained presentation, including an
absent batch, instead of relabelling it as current.
...
1. Use the registry facts validated by the prepass:
   `FAILED(STACK_REQUIRES_TWO_MEMBERS)`; then, for each
...
2. Use P-D15's registry/admission disposition already determined by the
   allocation-free prepass; processing never re-admits or changes the slice.
   `FAILED(FRAME_BUDGET)` performs no source acquisition or unit-owned
   allocation. Fixed reservations are never refunded based on a later
   disposition.
3. Acquire required members sequentially in ascending enabled series ID. BUSY
   ...
   and leaves only later series NOT_EVALUATED; every acquired member through it
   projects one terminal CURRENT_OBSERVATION. If no FAILED occurs, complete
   every required acquisition. EMPTY then outranks BUSY. If neither FAILED nor
   EMPTY occurred but any source is BUSY, construct no fresh PRESENTED_CONTENT:
   an identical retained `structure_key` yields wholesale STALE_BUSY as defined
   above; otherwise return `FAILED(SOURCE_BUSY)` with every required acquired
   member projected as one terminal CURRENT_OBSERVATION. NOT_EVALUATED may
   appear only after the first source FAILED or a pre-acquisition short-circuit.
...
5. Normalize every member under D2. `FAILED(NONFINITE_REJECTED)` precedes a
   ...
   only through `checked_cumulative_step`: each private immutable record owns the
   one ordinary double addition and is shared unchanged by composition, range,
   and indicator production. Convert its base/cumulative to the v1 float
   representation and require each converted float finite before emitting
   anything. Exact double-to-float roundtrip is not required; normal finite
   rounding is accepted. Failure is
...
After the final disposition is known, only READY constructs current
PRESENTED_CONTENT and only STALE_BUSY copies retained PRESENTED_CONTENT. EMPTY
and every failure in steps 3–7 project all acquired sources to their terminal
CURRENT_OBSERVATION, including an optional window already derived during a
successful READY selection, with no content/value/range/indicator payload.
...
`K*E`/visit count is checked. Reservation-formula arithmetic failure in the
metadata prepass makes the slice invalid and returns `FAILED(FRAME_BUDGET)`.
After admission, any actual-accounting arithmetic failure, prospective output/
visit/write excess, or `U_observed>U_limit` returns
...
The allocation-free metadata prepass charges the fixed checked reservation
`(M,V_limit,H_limit)` before table allocation or acquisition: every MAIN unit in
...
whose checked reservation is invalid/overflows or cannot fit every total returns
`FAILED(FRAME_BUDGET)`, emits no geometry, and performs no source or unit-owned
staging/geometry/QRhi/retained allocation. The later frame-wide public table
allocation contains its result record but is independently accounted and not
charged to that unit. Frame totals are consulted only while admitting the fixed
slices. Runtime work consumes only the admitted unit's own slice: actual use
never changes a shared remaining balance, transfers slack, refunds capacity, or
readmits a rejected unit. Consequently an earlier unit's low or full actual use
...
virtual size, or physical/OS-committed memory. Two exact counters have distinct
owners and transitions:

- `renderer_owned_requested_live_bytes` is the sole
  `STACK_CPU_BYTES_LIMIT` oracle. A successful counted allocation made under
  renderer ownership increments it by the exact allocator-request bytes; actual
  renderer-private deallocation or an explicit public-transfer boundary
  decrements it by those same bytes exactly once;
- `allocation_requested_live_bytes` is allocation-lifetime evidence. Every
  successful counted allocation increments it by the exact allocator-request
  bytes. Publication, handle copy/move, and non-final destruction do not change
  it; final deallocation decrements it by those same bytes exactly once.

A failed allocation increments neither counter. Allocator bookkeeping/rounding,
RSS, virtual size, and physical/OS-committed memory participate in neither. The
...
renderer-private allocation increments both counters at successful allocation;
only its actual deallocation decrements both exactly once. Dropping or copying
an internal alias is not transfer or deallocation. Separate process-memory
evidence records physical/RSS/commit behavior and never participates in either
exact requested-byte counter.
...
`STACK_CPU_BYTES_LIMIT` while renderer-owned; the ordinary D9 section and opaque
allocator overhead are excluded.

The private `stack_result_table_builder_t` uses one repo-local, table-specific
`stack_result_counted_backing_t` allocation/control primitive. It is not
`shared_ptr`, has no opaque control block, and consists of one header followed by
checked padding and the complete table payload. Before allocation, the builder
checked-computes every payload-region count, byte size, required alignment,
aligned offset, inter-region padding, total payload size, overall allocation
alignment, and exact allocator-request size. The header contains a thread-safe
atomic reference count plus immutable allocator-request size/alignment and every
payload offset/count/size needed by observers; it owns the payload lifetime.
Opaque allocator bookkeeping outside the requested block is not guessed or
charged.

The builder prospectively checks
`renderer_owned_requested_live_bytes` plus the exact allocator-request size
before allocation. Arithmetic failure or a total above
`STACK_CPU_BYTES_LIMIT` produces
`FAILED(RESULT_STORAGE_BUDGET)` in the enclosing section without allocating. If
the checked total is admitted but this single allocation fails, the section is
`FAILED(RESULT_STORAGE_ALLOCATION_FAILED)`. On success, the requested control
and payload bytes increment both exact counters once, every region is populated
privately, and all metadata/payload are frozen before publication; only the
atomic reference count remains mutable. No `sizeof(shared_ptr<...>)`, guessed
control size, or opaque allocator overhead participates in the cap oracle.

READY atomically installs the private geometry/resources and its value-only
retained presentation record; STALE_BUSY reuses that private geometry and
copies only the retained value record into the current public
`stack_result_entry_t`.

`stack_result_table_t` is a copyable immutable handle to this public-only
backing. Copy performs only a thread-safe reference increment; move transfers
the pointer; destruction performs only a reference decrement until the final
handle, with no allocation, payload copy, or offset rebasing. Atomic publication
transfers the renderer handle to GUI/public ownership and decrements
`renderer_owned_requested_live_bytes` exactly once without allocation or a
change to `allocation_requested_live_bytes`. Copy/move and non-final destruction
change neither counter and preserve the backing. Final-handle destruction
destroys the frozen payload regions and counted header, deallocates the exact
allocator request, and decrements `allocation_requested_live_bytes` exactly
once. Publication does not pretend the allocator request was destroyed, and
final deallocation cannot debit the renderer-owned counter a second time.

The public stack-result table is the only intentionally transferred counted
allocation in this contract. Any future intentionally transferred counted
allocation must use this same explicit public-table transition: atomically
decrement `renderer_owned_requested_live_bytes` at ownership transfer, leave
`allocation_requested_live_bytes` unchanged through aliases/copies/non-final
destruction, and decrement it only at actual final deallocation. A private
retained/staging allocation cannot be exported by calling an alias a transfer.
...
raw group/member pointers, and no public backing/view references private
renderer state. A view remains valid while at least one handle owns that backing
and provides no independent lifetime extension. Ordinary D9 result storage and
Stage 3 per-series arrays remain excluded from `STACK_CPU_BYTES_LIMIT`. Other
capped stack CPU storage uses only a narrow repo-local noncopyable exact-size
owning contiguous array—equivalent to
...
committed bytes remain outside both exact requested-byte counters.
...
- one checked cumulative-step addition per emitted position and immutable
  presented sequence consumed by geometry/GPU/VISIBLE, plus cursor sampling of
...
CURRENT_OBSERVATION, and final-disposition-only PRESENTED_CONTENT origins; and
one-add double/float-max/absorption/signed-zero/negative/cancellation/ordinary-
rounding representability with no partial emit.
...
BUSY publishes each required member's terminal CURRENT_OBSERVATION as
`FAILED(SOURCE_BUSY)`.
Generation tests prove every enclosing result has current publication identity,
current-attempt `cursor_state_id`, READY has current content frame/cursor
identity, EMPTY/FAILED have no content identity, and STALE has the current outer
publication plus wholly retained old content cursor identity, value-only public
entries/range, and optional indicator-batch state (present or absent), while
geometry remains private. Table tests prove
all MAIN records precede ordered PREVIEW records, each class orders groups by
...
one belonging to a `FAILED(reason)` group/view or having `NOT_EVALUATED` origin,
carries `series_id`. Per-series records never have a failure
disposition. Delayed result and
...
terminal attempted LOD/observed sequence, has a window only after successful
window derivation, and claims neither full desired-sample inspection nor
derivation success or value/content; `observed_empty` preserves its controlling
terminal LOD and exact sequence including zero with no window; `observed_busy`
and `observed_failed` preserve their controlling terminal LOD with sequence zero
and no window.
`UNSUPPORTED` cannot construct it.
PRESENTED_CONTENT exists only inside READY/STALE_BUSY and carries the selected
LOD/window from the complete plan with exact `content_frame_id` and
`content_cursor_state_id`. Attempted and selected fields cannot substitute for
one another. It is constructed only after final READY or copied only after final
STALE_BUSY. Final EMPTY/every FAILED projects each acquired member—including a
selected READY with derived window—to terminal CURRENT_OBSERVATION with no
content ID/value/range/indicator. Every earlier LOD attempt and all visit
counters exist only in the complete internal diagnostic trace/profiler tagged
by group/view/series/attempt. Retained `content_cursor_state_id` remains old even
without a batch and is rejected against a newer active cursor state; deactivation
does not rewrite it.
Focused failures cover READY acquisition followed by prospective visit
exhaustion, unordered input, an LOD-budget decision that prevents selection,
window failure, and another derivation failure; each retains only the terminal
public `observed_ready` facts after
writer progress proves the hold is gone. The complete internal attempt trace
retains every earlier attempt/counter, stops at the prospective limit, proves no
scan past it, and cannot be conflated with or copied into the public outcome.
Final-disposition tests acquire/select READY members, then produce empty
intersection, nonfinite, fragmentation, numeric, output-budget, resident, and
RHI-allocation outcomes; every acquired member is terminal CURRENT_OBSERVATION
with any genuinely derived window and none is PRESENTED_CONTENT.
...
terminal-attempted-or-selected LOD/window, rendered range,
`optional<stack_indicator_batch_t>`, `content_frame_id`, and
`content_cursor_state_id`; the enclosing result alone carries current-attempt
...
presented payload with content frame/cursor IDs and the exact optional indicator
batch. Every present batch request ID equals `content_cursor_state_id`. No
request requires absence; a request covers every enabled member including
`NONE` and produces
exactly `K>=2` ascending items with the ID once at batch level. The batch is
uniformly READY with factory-derived finite component/base/cumulative fields and
finite total, or uniformly EMPTY with only series IDs and no total value. EMPTY,
every
`FAILED(reason)`, and post-composition numeric/resident/CPU/RHI allocation
failure expose no content frame/cursor ID, rendered range, or indicator batch,
and every acquired member is terminal CURRENT_OBSERVATION.
`Stack_indicator_status`/value combinations and invalid origin/status/sequence/
window combinations are compile-time
unconstructible.
...
`series_id`, one batch-level `cursor_request_id` equal to presented cursor state,
and private ordered cursor-sampled READY records from the frozen presented
sequence. Emitted checked-step records are the sole geometry/GPU/VISIBLE truth;
cursor evaluation changes no emitted position, geometry, `M`/`V_limit`/
`H_limit`, admission, or acquisition. Candidate A tests exact emitted and
between-event evaluation under its normalized/event-union duplicate/nonfinite/
gap/hold rules, before/at/after STEP, and exact cursor/render parity. Candidate B
tests exact-grid and between-grid evaluation, before/at/after displaced STEP
transitions, narrow spikes, gaps, and holds,
always against finalized grid output rather than raw/pre-grid data. Both
independently interpolate stored component/base/cumulative fields with no
cumulative add/reconstruction. Exact emitted samples may reuse records; frozen
gap/out-of-domain returns the complete all-EMPTY batch. Absorption `1e16+1`
preserves component `1` with cumulative `1e16`; signed-zero, negative,
cancellation, and ordinary-rounding cases prove canonical first base, bitwise
adjacency, final-total identity, and pixel markers at cumulative. Public callers
cannot manufacture records or inconsistent component/base/cumulative/total.
EMPTY items/total are valueless. Factories reject zero length, filtered subsets,
...
every result order. After config validation, inactive state ignores every
delayed/retained result. Active matching current-attempt ID allows EMPTY/FAILED/
section-failure clears; active matching content ID allows READY/STALE apply or
no-batch clear. Cases include movement→BUSY stale/no batch/group failure/section
failure, matching/mismatching stale, deactivation, and proof that content never
reappears.
...
API tests prove both result-storage reasons are section dispositions only and
cannot construct, serialize as, or compare equal to an entry failure reason.

Prepass gates prove an allocation-free deterministic traversal completes all
MAIN/PREVIEW group/member positions, registry failures, and admission slices/
dispositions before the single frame-wide table allocation, which precedes
acquisition. Registry-invalid and reservation-invalid/no-fit units have the
required pre-acquisition result and no source or unit-owned staging/geometry/
QRhi/retained allocation. The independently counted frame table still contains
every rejected unit. Phase counters enforce
`metadata/admission < table allocation < acquisition`.

Lifetime gates prove `stack_result_table_t` is a copyable immutable handle:
publication, move, and every copy retain one counted backing identity; copy
performs only atomic increment with zero allocation/payload duplication, move
transfers only its pointer, and destruction atomically decrements. Cross-thread
copy/destruction stress preserves stable offset-derived views through every non-
final release; final release destroys/deallocates and debits exactly once.
Address/ownership probes prove no public backing or view aliases private
renderer state.

Backing-layout/cap gates prove checked header size/alignment, every padding/
...
`renderer_owned_requested_live_bytes`; it excludes allocator bookkeeping,
size-class/OS rounding, and physical/OS-committed memory and contains no guessed
...
Counter-transition gates prove a successful public-table allocation increments
both `renderer_owned_requested_live_bytes` and
`allocation_requested_live_bytes`; publication atomically decrements only the
renderer-owned counter; copies and non-final destruction change neither; final
deallocation decrements only the allocation-lifetime counter. Failed allocation
changes neither, and destruction-before-publication plus renderer-private
allocation/deallocation debit both exactly once without double debit.
No-export gates prove retained/staging/private backing exposes no external owner;
all internal alias/copy/drop operations retain renderer ownership and change
...
construction counts the exact table-specific header/padding/payload allocator-
request bytes before `COMPLETE` P-R1 entries become available and performs no
post-install table allocation.
...
- multi-group scenarios use identical proposed P-D15 order and
  `FRAME_M_LIMIT`/`FRAME_V_LIMIT`/`FRAME_H_LIMIT`; a deterministic allocation-
  free prepass maps every position/disposition before one frame table
  allocation. Rejected `(group,view)` units report `FAILED(FRAME_BUDGET)`,
  perform no source or unit-owned allocation, and leave no partial geometry;
...
- both candidates call the one private `checked_cumulative_step` per member/
  position and feed its unchanged immutable records to composition conversion,
  VISIBLE range, and the frozen private presented sequence. Their cursor
  evaluators sample component/base/cumulative from that sequence independently
  with no cumulative arithmetic, source query, new position, or budget/admission/
  acquisition change. Absorption/signed-zero/negative/cancellation/ordinary-
  rounding replays prove component preservation and cursor/render pixel parity;
- common-adapter tests prove P-D15's stack-exclusive CPU scope, exact requested
  ...
  retained geometry plus value-record ownership with no external export,
  internal-alias no-transition behavior and actual-deallocation-only dual debit,
  public-table-only intentional transfer semantics, the one exact repo-local
  counted backing/control allocation, checked header/padding/payload layout,
  exact allocator-request `renderer_owned_requested_live_bytes` cap accounting,
  allocation-lifetime `allocation_requested_live_bytes` evidence, their exact
  allocate/transfer/copy/non-final/final/fault transitions with no double debit,
  and the allocation-fault split; exact requested element/storage bytes for
  every other capped
  ...
  entry behavior and stale suppression after every budget/allocation failure,
  configured-cap
...
- `RU-4B-B` freezes Candidate B's cursor evaluator against the finalized grid,
...
- `STACK_PUBLIC_ACTIVATION` owns metadata/builder/registry reachability,
  ...
  frame-tagging/sequences, enclosing/presented cursor-state propagation,
  retained stale cursor rejection even without a batch, exact public group/
  ...
  observers, the exact four terminal source-observation factories/timing, one
  public controlling outcome versus complete internal attempt trace, the closed
  `Stack_failure_reason` membership/disposition boundary, the exact
  private one-add cumulative-step producer/record and
  `Stack_indicator_status`/optional-batch factories with copied bitwise
  adjacency/total rules, the complete GUI config/activation/cursor transition
  matrix, final-disposition origin projection, and only the P-R1 value
  fields—no private cumulative record, visit-counter/earlier-attempt/geometry/
  span/sample/resource/source-identity/owner/key fact.
  Construction/publication/lifetime tests prove the table's public-only backing
  is the single table-specific counted header/padding/payload allocation with
  exact checked requested bytes and no `shared_ptr`/opaque control block. It
  freezes before publication, never aliases private backing, moves without
  allocation/dangling views, and copies by thread-safe refcount only. Observers
  derive views from validated offsets/counts without stored raw pointers;
  allocation, publication, copy/non-final destruction, final deallocation, and
  fault paths follow the exact two-counter lifetime/debit contract without
  double debit. Private retained/staging backing exposes no external owner;
  internal aliases change neither counter and only actual deallocation debits
  both. Every intentional transfer uses the public-table transition.
  Integration counters prove metadata/admission precedes table allocation,
  which precedes acquisition, and rejected units allocate no unit resource.
  The unit also owns normative API/migration/package docs and install/package
  consumer smoke on the same source hash;
...
- BUSY compares only `structure_key`; P-R1 STALE_BUSY copies the prior complete
  tagged presentation wholesale—including old `content_cursor_state_id` even
  without a batch—exposes the current BUSY only at group level, and leaves
  current partial attempts in trace/counters. With no retained match,
...
- exhaustive result variants make every invalid payload combination
  ...
  RHI-allocation failures expose no content frame/cursor identity, rendered
  range, or indicator batch;
- CURRENT_OBSERVATION has exactly the four source-status factories and their
  terminal-attempted-LOD/sequence/window shapes. READY attempt/consume-scope
  termination and hold destruction precede `observed_ready` even when visit/
  order/LOD-budget/window/other derivation later fails. Exactly one public
  controlling outcome is final READY presentation, retained final STALE
  presentation, or terminal READY/EMPTY/BUSY/FAILED observation; earlier
  attempts and all visit counters remain in the
  complete internal tagged trace, where prospective-limit and writer-progress
  tests prove no scan past the limit without conflating trace and public result.
  The public outcome implies neither complete desired-sample inspection nor
  derivation/value success.
  UNSUPPORTED and every invalid status/sequence/window combination are
  unconstructible. PRESENTED_CONTENT is final READY/STALE only. Selected READY
  followed by empty intersection, nonfinite, fragmentation, numeric, output,
  resident, CPU, or RHI failure projects every acquired source to terminal
  CURRENT_OBSERVATION with any derived window but no presented payload;
- `Stack_failure_reason` membership is exact and result-storage dispositions
  ...
  all-READY/all-EMPTY domain; private one-add emitted records shared by geometry/
  GPU/VISIBLE; frozen A event-union and B finalized-grid cursor evaluators; exact
  emitted and between-position samples; before/at/after STEP, spike, gap, hold,
  absorption `1e16+1`, signed zero, ordinary rounding, negative/cancellation,
  ...
  Public record manufacture and zero/filtered/mixed/inconsistent batches are
  unconstructible;
...
- after the sole checked-step double addition, convert its base/cumulative to v1 float and require the
...
- stack-exclusive CPU and exact requested QRhi byte instrumentation proves both
  ...
  transaction;
- phase instrumentation proves the allocation-free registry/admission prepass
  completes every deterministic position/disposition before the single frame-
  wide table allocation, which precedes all acquisition. Registry/reservation-
  rejected units have no source or unit staging/geometry/QRhi/retained
  allocation; the shared table charge remains frame-owned;
- the narrow exact-size owning array proves exact count/bytes and ownership
  ...
  allocated public stack table that never aliases private backing, and the one
  table-specific counted backing primitive. Gates cover checked header/padding/
  every payload region offset/size/alignment, exact allocator-request bytes with
  opaque overhead excluded, prospective cap at the exact limit and limit+1,
  configured-cap rejection, admitted allocator faults, freeze-before-
  ...
  and final destruction deallocates and decrements
  `allocation_requested_live_bytes` exactly once. Successful allocation
  increments it and `renderer_owned_requested_live_bytes`; publication
  atomically decrements only the renderer-owned cap oracle, while copies and
  non-final destruction change neither. Failed allocations change neither.
  Private retained/staging backing never exports; internal aliases leave both
  counters unchanged and only actual private deallocation decrements both. Any
  intentional transfer follows the public-table renderer-at-transfer/allocation-
  at-final-deallocation split, and no path double-debits. Allocator bookkeeping/
  rounding and physical/OS-committed
  process memory remain separately evidenced and excluded. Every other capped
  stack array is charged by exact requested element/storage bytes;
- structural-result probes exhaust every section/entry/member factory and
  ...
  mutation. The four terminal CURRENT_OBSERVATION source statuses, terminal-
  attempted-versus-selected LOD/window fields, internal attempt-trace boundary,
  exact `Stack_failure_reason` membership, and
  section-only result-storage dispositions are exhaustive. READY/STALE_BUSY
  alone expose content frame/cursor ID, range, and the exact optional indicator
  batch; absent/
  all-READY/all-EMPTY/discontinuity/delayed/stale cases satisfy the closed
  exactly-K private cursor-sampled batch/item factories and copied bitwise-field
  rules, while emitted checked-step records remain private and public record
  construction and zero/filter/mixed/inconsistent states cannot compile;
  EMPTY and every failure, including post-composition numeric, resident-cap,
  CPU-allocation, and RHI-allocation failures, expose none and project every
  acquired source as terminal CURRENT_OBSERVATION;
