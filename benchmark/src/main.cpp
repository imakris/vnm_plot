...
constexpr int EXIT_RUNTIME_ERROR = 2;

void print_version()
{
    std::cout << "vnm_plot_benchmark version " << VERSION << "\n";
}
...
std::string validate_config(const Benchmark_config& config)
{
...
void print_config_summary(const Benchmark_config& config, std::ostream& os)
{
    os << "Configuration:\n"
...
int main(int argc, char* argv[])
{
    ...
    if (!config.quiet) {
        std::cout << "vnm_plot Benchmark v" << VERSION << "\n"
                  << std::string(30, '=') << "\n\n";
    ...
    Benchmark_window window(config);

    // Track exit code
    int exit_code = EXIT_SUCCESS_CODE;

    // Connect finish signal to generate report and quit
    QObject::connect(&window, &Benchmark_window::benchmark_finished, [&]() {
    ...
    return (app_result != 0) ? EXIT_RUNTIME_ERROR : exit_code;
}
